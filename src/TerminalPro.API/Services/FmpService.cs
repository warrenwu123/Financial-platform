...
public class FmpService : IFmpService
{
    ...
    public async Task<List<FmpIncomeStatement>> GetIncomeStatementsAsync(
        string symbol, string period = "annual", int limit = 5) =>
        await GetAsync<List<FmpIncomeStatement>>(
            Url($"/v3/income-statement/{symbol}", $"&period={period}&limit={limit}"),
            TimeSpan.FromHours(6)) ?? [];

    public async Task<List<FmpBalanceSheet>> GetBalanceSheetsAsync(
        string symbol, string period = "annual", int limit = 5) =>
        await GetAsync<List<FmpBalanceSheet>>(
            Url($"/v3/balance-sheet-statement/{symbol}", $"&period={period}&limit={limit}"),
            TimeSpan.FromHours(6)) ?? [];

    public async Task<List<FmpCashFlowStatement>> GetCashFlowsAsync(
        string symbol, string period = "annual", int limit = 5) =>
        await GetAsync<List<FmpCashFlowStatement>>(
            Url($"/v3/cash-flow-statement/{symbol}", $"&period={period}&limit={limit}"),
            TimeSpan.FromHours(6)) ?? [];

    public async Task<List<FmpKeyMetrics>> GetKeyMetricsAsync(
        string symbol, string period = "annual", int limit = 5) =>
        await GetAsync<List<FmpKeyMetrics>>(
            Url($"/v3/key-metrics/{symbol}", $"&period={period}&limit={limit}"),
            TimeSpan.FromHours(6)) ?? [];

    public async Task<List<FmpRatios>> GetRatiosAsync(
        string symbol, string period = "annual", int limit = 5) =>
        await GetAsync<List<FmpRatios>>(
            Url($"/v3/ratios/{symbol}", $"&period={period}&limit={limit}"),
            TimeSpan.FromHours(6)) ?? [];

    public async Task<FmpProfile?> GetProfileAsync(string symbol) =>
        (await GetAsync<List<FmpProfile>>(
            Url($"/v3/profile/{symbol}"), TimeSpan.FromHours(24)))?.FirstOrDefault();
}
